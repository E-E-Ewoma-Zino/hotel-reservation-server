...
module.exports = {
	async createRoom(req, res) {
		...
		try {
			if (bodyData?.features || bodyData?.features !== '') bodyData.features = bodyData.features.split(',');
			console.log("new rooms", bodyData);

			// upload to cloudinary
			uploadToCloudinary([...bodyData.images, bodyData.videos], async (result) => {
				console.log("our result", result);
				let count = 0;
				while(count <= bodyData.images.length - 1){
					bodyData.images.cloud = result[count];
					count++;
				}
				while((count - (bodyData.images.length - 1)) <= bodyData.videos.length - 1){
					bodyData.videos.cloud = result[count];
				}
